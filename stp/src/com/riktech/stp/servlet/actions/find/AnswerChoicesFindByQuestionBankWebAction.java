...
public class AnswerChoicesFindByQuestionBankWebAction extends WebAction
{
	/**
	 * Method 'execute'
	 * 
	...
	public void execute(WebController servlet, HttpServletRequest request, HttpServletResponse response) throws ServletException
	{
		try {
			...
			// execute the finder
			ArrayList<AnswerChoices> list= dao.findByQuestionBank(nextQuestId);
		
			// store the results
			request.setAttribute( "result", list );
		
			// forward request on to the appropriate JSP page to display the results
			forward( servlet, request, response, "/find/AnswerChoicesFindByQuestionBankResult.jsp" );
		}
		catch (Exception e) {
